...
import org.sea.chat.exception.BizError;
import org.sea.chat.utils.JsonUtils;
...
public class WebSocketHandler extends SimpleChannelInboundHandler<TextWebSocketFrame> {
    ...
    protected void channelRead0(ChannelHandlerContext channelHandlerContext, TextWebSocketFrame textWebSocketFrame) {
        ChatServerReqMsg chatServerReqMsg = JsonUtils.jsonToBean(textWebSocketFrame.text(), ChatServerReqMsg.class);
        if (Objects.isNull(chatServerReqMsg)) {
            channelHandlerContext.channel().writeAndFlush(BizError.B_MESSAGE_UNDEFINED.getErrDesc());
            throw new BizException(BizError.B_MESSAGE_UNDEFINED.getErrDesc());
        }
